...
function zert_log {
    local color="$1" msg="$2" stderr="$3"
    local red="\033[31m" blue="\033[34m" reset="\033[0m"
    local prefix="[ZERT]: "
    case "$color" in
        red)   echo -e "${red}${prefix}${msg}${reset}" >${stderr:-/dev/stderr} ;;
        blue)  echo -e "${blue}${prefix}${msg}${reset}" ;;
        *)     echo -e "${prefix}${msg}" ;;
    esac
}
...
if ! command -v git >/dev/null 2>&1; then
    zert_log red "git is required to install Zert" stderr
    return 1
fi

# Create plugins directory
mkdir -p "$ZERT_PLUGINS_DIR" || {
    zert_log red "Failed to create $ZERT_PLUGINS_DIR" stderr
    return 1
}
...
git clone "https://github.com/whoiscircuit/zert" "$ZERT_PLUGINS_DIR/zert" 2>&1 || {
    zert_log red "Failed to clone Zert to $ZERT_PLUGINS_DIR/zert" stderr
    return 1
}
